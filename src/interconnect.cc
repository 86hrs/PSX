...
Interconnect::Interconnect(Bios *p_bios, RAM *p_ram, Dma *p_dma, GPU *p_gpu)
...
void Interconnect::do_dma(Port p_port) {
  if (this->dma->get_mut_channel(p_port).get_sync() == Sync::LinkedList) {
...
void Interconnect::do_dma_block(Port p_port) {
  ...
  uint32_t remsz;
  if (auto transfer_size = channel.transfer_size(); transfer_size.has_value()) {
    remsz = *transfer_size;
  } else {
...
uint32_t Interconnect::dma_reg(uint32_t p_offset) {
  ...
  case 0 ... 6: {
    ...
    Channel &channel = this->dma->get_mut_channel(major_port);
    if (minor == 8) {
...
void Interconnect::store16(uint32_t p_addr, uint16_t p_val) {
  ...
  // IQR
  if (auto offset = map::IQR_CONTROL.contains(addr); offset.has_value()) {
  ...
  // SPU Registers
  if (auto offset = map::SPU.contains(addr); offset.has_value()) {
...
void Interconnect::store8(uint32_t p_addr, uint8_t p_val) {
  ...
  // EXPANSION 2
  if (auto offset = map::EXPANSION_2.contains(addr); offset.has_value()) {
...
void Interconnect::store32(uint32_t p_addr, uint32_t p_val) {
  ...
  if (auto offset = map::IQR_CONTROL.contains(p_addr); offset.has_value()) {
    printf("IQR write: 0x%x\n", p_val);
    return;
  }
  // GPU
  if (auto offset = map::GPU_GP0.contains(p_addr); offset.has_value()) {
    this->gpu->gp0(p_val);
    return;
  }
  if (auto offset = map::GPU_GP1.contains(p_addr); offset.has_value()) {
    this->gpu->gp1(p_val);
    return;
  }
  // TIMER
  if (auto offset = map::TIMER_1.contains(p_addr); offset.has_value()) {
    printf("TIMER1 write: 0x%x\n", p_val);
    return;
  }
  if (auto offset = map::TIMER_2.contains(p_addr); offset.has_value()) {
    printf("TIMER2 write: 0x%x\n", p_val);
    return;
  }
  if (auto offset = map::TIMER_0.contains(p_addr); offset.has_value()) {
    printf("TIMER0 write: 0x%x\n", p_val);
    return;
  }
  // DMA
  if (auto offset = map::DMA.contains(p_addr); offset.has_value()) {
...
uint32_t Interconnect::load32(uint32_t p_addr) {
  ...
  // EXPANSION 1
  if (auto offset = map::EXPANSION_1.contains(p_addr); offset.has_value()) {
