...
Bios::Bios(const char *filename) {
    ...
    if (!ifs)
        throw std::runtime_error(std::string("Failed to open BIOS file: ") +
                                 filename);
    ...
    if (file_size != EXPECTED_SIZE) {
        throw std::runtime_error("Invalid BIOS file size. Expected " +
    ...
    if (!ifs.read(reinterpret_cast<char *>(this->data.data()), EXPECTED_SIZE)) {
