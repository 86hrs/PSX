...
void GPU::gp1(uint32_t p_val) {
  ...
  switch (opcode) {
  case 0x02:
    this->gp1_acknowledge_irq();
    break;
  case 0x01:
    this->gp1_reset_command_buffer();
    break;
  case 0x03:
...
void GPU::gp0_quad_mono_opaque() { printf("GP0: Draw mono opaque quad\n"); }

void GPU::gp0_quad_shaded_opaque() { printf("GP0: Draw shaded opaque quad\n"); }
...
void GPU::gp1_dma_direction(uint32_t p_val) {
  static constexpr DmaDirection directions[] = {
      DmaDirection::Off, DmaDirection::Fifo, DmaDirection::CpuToGp0,
