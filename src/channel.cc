...
void Channel::set_control(uint32_t p_val) {
  // bit 0: direction
  this->direction = (p_val & 1) ? Direction::FromRam : Direction::ToRam; // ok

  // bit 1: address step  (FIXED — was using bit 8)
  this->step =
      ((p_val >> 1) & 1) ? Step::Decrement : Step::Increment; // <-- fix
...
void Channel::set_base(uint32_t p_val) { this->base = p_val & 0xffffff; }
...
bool Channel::active() {
  if (this->sync == Sync::Manual)
    return this->enable && this->trigger;
  else
